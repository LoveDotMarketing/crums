...
const FacebookLanding = () => {
  ...
  const handleFormFocus = (fieldName: string) => {
    if (!formStarted) {
      setFormStarted(true);
      trackFormStart("facebook_landing", fieldName);
    }
  };
      ...
      const { data, error } = await supabase.functions.invoke("send-contact-email", {
        body: {
          ...
          message: formData.message
            ? `[Facebook Ad Lead] Trailers needed: ${formData.trailersNeeded || "Not specified"}. ${formData.message}`
            : `[Facebook Ad Lead] Trailers needed: ${formData.trailersNeeded || "Not specified"}`,
          ...leadSourceData,
