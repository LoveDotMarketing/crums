...
import { trackFormStart, trackPageView, setGoogleAdsUserData, splitName } from "@/lib/analytics";
...
const LinkedInLanding = () => {
  ...
  const handleFormFocus = (fieldName: string) => {
    if (!formStarted) {
      setFormStarted(true);
      trackFormStart("linkedin_landing", fieldName);
    }
  };
      ...
      const { data, error } = await supabase.functions.invoke("send-contact-email", {
        body: {
          ...
          message: formData.message
            ? `[LinkedIn Ad Lead] Trailers needed: ${formData.trailersNeeded || "Not specified"}. ${formData.message}`
            : `[LinkedIn Ad Lead] Trailers needed: ${formData.trailersNeeded || "Not specified"}`,
          ...leadSourceData,
